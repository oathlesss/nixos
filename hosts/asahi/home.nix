  ...
  home = {
    ...
    packages =
      let
        ...
        hasTtf =
          if fontDirExists then
            builtins.any (name: lib.hasSuffix ".ttf" name) (builtins.attrNames (builtins.readDir fontDir))
          else
            false;
      in
      (
        if hasTtf then
          [
            (
              let
                fontDirPath = builtins.path {
                  path = fontDir;
                  name = "berkeley-mono-nerd";
                };
              in
        ...
        else if canInspectParent then
          lib.warn
            "BerkeleyMono Nerd Font not found at ${fontDir} — skipping font install. Patch and copy TTFs there to enable it."
            [ ]
        else
          [ ]
      )
    ...
    git = {
      ...
      settings = {
        ...
        "color \"blame\"".highlightRecent =
          "black bold,1 year ago,white,1 month ago,default,7 days ago,blue";
        "color \"branch\"" = {
          current = "magenta";
          local = "default";
          remote = "yellow";
